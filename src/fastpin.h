...
class Pin : public Selectable { 
	...
	void _init() {
		mPinMask = _BV(0); // digitalPinToBitMask(mPin);
		mPort = &PORTB; //portOutputRegister(digitalPinToPort(mPin));
	}
public:
	...
	inline void setOutput() { DDRB |= mPinMask; } //pinMode(mPin, OUTPUT); }
	inline void setInput() { DDRB &= ~mPinMask; } //pinMode(mPin, INPUT); }
...
template<uint8_t PIN> class FastPin { 
	...
	static void _init() { 
		sPinMask = _BV(0); //sPinMask = digitalPinToBitMask(PIN);
		sPort = &PORTB; //sPort = portOutputRegister(digitalPinToPort(PIN));
	}
public:
	typedef volatile uint8_t * port_ptr_t;
	typedef uint8_t port_t;

	inline static void setOutput() { _init(); DDRB |= sPinMask; } //pinMode(PIN, OUTPUT); }
	inline static void setInput() { _init(); DDRB &= ~sPinMask; } //pinMode(PIN, INPUT); }
